...
This branch is the 3.0 beta, a Gallery prerelease. See [BETA.md](BETA.md) for what changed.

```powershell
Install-Module Netscoot -Scope CurrentUser -AllowPrerelease   # PowerShellGet (Windows PowerShell 5.1+ / PowerShell 7)
Install-PSResource Netscoot -Prerelease                       # PSResourceGet (the newer installer, PowerShell 7.4+)
...
A second setting, the update channel, decides which releases the check and `Update-Netscoot` offer.
`Stable` (the default) offers only stable releases, and `Beta` offers prereleases too:

```powershell
Set-NetscootUpdateChannel -Channel Beta -Scope User   # offer betas (persisted for your user on Windows)
Get-NetscootUpdateChannel                              # show the effective channel and where it came from
Set-NetscootUpdateChannel -Channel Stable -Scope User # back to stable releases only
```

The channel is stored in the `NETSCOOT_CHANNEL` environment variable. A Gallery install takes betas
through `Update-Module Netscoot -AllowPrerelease` instead of the channel.
...
- **Only when you ask:** `Register-NetscootGitAlias` adds one `alias.netscoot` line to your git
  ...
  `Clear-NetscootJournal` deletes a repository's journal. `Set-NetscootUpdatePolicy` and
  `Set-NetscootUpdateChannel` set the `NETSCOOT_AUTOUPDATE` and `NETSCOOT_CHANNEL` environment
  variables, persisted for your user or the machine on Windows when you ask.
...
| `NETSCOOT_CHANNEL` | `beta` | Backs the update channel (see [Updating](#updating)): `beta` offers prereleases, unset = Stable. Prefer `Set-NetscootUpdateChannel`. |
...
> [!NOTE]
> **For sysadmins.** These controls are fleet-settable. `NETSCOOT_CHANNEL` works the same way as
> the update policy. Set `NETSCOOT_AUTOUPDATE` machine-wide via
...
| [Get-NetscootSolutionInventory](#get-netscootsolutioninventory) | [Get-NetscootSolutionInventory](#get-netscootsolutioninventory) [[`-RepositoryRoot`] &lt;string&gt;] [&lt;CommonParameters&gt;] |
