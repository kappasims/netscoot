...
Describe 'Move journal + Undo-Netscoot' {
    It 'journals a move in the per-user store and Undo reverses it, popping the entry' {
        ...
        try {
            ...
            Undo-Netscoot -RepoRoot $root -Confirm:$false | Out-Null
    ...
    It 'Invoke-Netscoot -NoJournal forwards the per-call opt-out to the engine' {
        ...
        try {
            $lib = Join-Path $root (Join-Path 'src' (Join-Path 'Lib' ('Lib.csproj')))
            Invoke-Netscoot -Path $lib -Destination (Join-Path $root (Join-Path 'libs' ('Lib'))) -RepoRoot $root -NoBuild -NoJournal -Confirm:$false | Out-Null
            (Join-Path $root (Join-Path 'libs' (Join-Path 'Lib' ('Lib.csproj')))) | Should -Exist
            @(Get-MoveJournalEntries -RepoRoot $root).Count | Should -Be 0
        } finally { Remove-Item -LiteralPath $root -Recurse -Force -ErrorAction SilentlyContinue }
    }
    ...
    It 'Set-NetscootJournal -Enabled $false (git config) turns journaling off and back on' {
        ...
        try {
            ...
            Set-NetscootJournal -Enabled $false -RepoRoot $root -Confirm:$false | Out-Null
            ...
            Set-NetscootJournal -Enabled $true -RepoRoot $root -Confirm:$false | Out-Null
    ...
    It 'Clear-NetscootJournal deletes the journal and empties the undo history' {
        ...
        try {
            ...
            Clear-NetscootJournal -RepoRoot $root -Confirm:$false | Out-Null
            (Get-MoveJournalPath -RepoRoot $root) | Should -Not -Exist
            @(Get-MoveJournalEntries -RepoRoot $root).Count | Should -Be 0
        } finally { Remove-Item -LiteralPath $root -Recurse -Force -ErrorAction SilentlyContinue }
    }
    ...
    It 'Undo-Netscoot -List shows entries and -WhatIf changes nothing' {
        ...
        try {
            ...
            @(Undo-Netscoot -RepoRoot $root -List).Count | Should -Be 1
            Undo-Netscoot -RepoRoot $root -WhatIf | Out-Null
    ...
    It 'Undo-Netscoot -All -Force reverses every move and empties the journal' {
        ...
        try {
            ...
            Undo-Netscoot -RepoRoot $root -All -WhatIf | Out-Null
            $vendor | Should -Exist
            @(Get-MoveJournalEntries -RepoRoot $root).Count | Should -Be 2

            Undo-Netscoot -RepoRoot $root -All -Force | Out-Null
    ...
    It 'writes a non-terminating error when there is nothing to undo' {
        $root = New-JournalFixture
        try {
            Undo-Netscoot -RepoRoot $root -ErrorVariable errs -ErrorAction SilentlyContinue | Out-Null
    ...
    It 'refuses to replay a tampered journal whose command is not a recognized mover' {
        ...
        try {
            ...
            { Undo-Netscoot -RepoRoot $root -Confirm:$false } | Should -Throw -ExpectedMessage '*not a recognized*'
        } finally { Remove-Item -LiteralPath $root -Recurse -Force -ErrorAction SilentlyContinue }
    }
    ...
    It 'refuses to replay a tampered journal whose path escapes the repository' {
        ...
        try {
            ...
            Set-Content -LiteralPath $jp -Value ($e | ConvertTo-Json -Depth 8 -Compress) -Encoding utf8
            { Undo-Netscoot -RepoRoot $root -Confirm:$false } | Should -Throw -ExpectedMessage '*outside the repository*'
        } finally { Remove-Item -LiteralPath $root -Recurse -Force -ErrorAction SilentlyContinue }
    }
}
