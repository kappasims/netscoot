...
Describe 'Sync-NetscootSolution' -Tag 'Integration' {
    ...
    It 'leaves solutions that share no project alone' {
        ...
        try {
            ...
            Sync-NetscootSolution -RepositoryRoot $root -Confirm:$false | Should -BeNullOrEmpty
    ...
    It 'warns about a missing .vcxproj instead of adding it through the dotnet CLI' {
        ...
        try {
            ...
            Mock -ModuleName Netscoot.Core Invoke-Dotnet { }
            Sync-NetscootSolution -RepositoryRoot $root -Confirm:$false -WarningVariable w -WarningAction SilentlyContinue | Out-Null
            Should -Invoke -ModuleName Netscoot.Core Invoke-Dotnet -Times 0 -Exactly
...
Describe 'Test-NetscootSolutionConsistency' {
    It 'runs without the dotnet CLI on PATH' {
        ...
        try {
            ...
            $r = @(Test-NetscootSolutionConsistency -RepositoryRoot $root -WarningAction SilentlyContinue)
            $r.Count | Should -Be 1
