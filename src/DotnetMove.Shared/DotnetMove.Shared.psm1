...
# Shared, cross-cutting helpers consumed by the engine modules (Core/Unity/Native) via
# RequiredModules. Split into Common (platform/paths/git/plan/capability) and Dotnet (the
# .NET/MSBuild helpers). Every function defined here is exported so the engines can call it.
